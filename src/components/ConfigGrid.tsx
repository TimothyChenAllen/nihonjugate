...
const ConfigGrid: React.FC<Props> = ({ verbs, onUpdate }) => {
  ...
  const handleGroupAction = async (groupVerbs: Verb[], setActive: boolean) => {
    ...
    await Promise.all(verbsToUpdate.map(v => 
  ...
  const getMasteryColor = (v: Verb) => {
    ...
    if (v.attempt_count === 0) return 'bg-slate-700 border-slate-600 text-white'; 
    
    const rate = v.correct_count / v.attempt_count;
    if (rate >= 0.9 && v.attempt_count > 3) return 'bg-emerald-900 border-emerald-500 text-emerald-100'; 
            ...
            <div className="p-3 bg-slate-900/50 border-b border-slate-800 flex justify-between items-start">
              <div>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-slate-200">{kanji}</span>
                  <span className="text-slate-500 text-sm font-mono">({forms[0].dictionary_kana})</span>
                </div>
                <div className="text-[10px] uppercase tracking-wider text-slate-600 font-bold mt-1">{forms[0].meaning}</div>
              </div>
              ...
              {forms.map(v => (
                ...
                >
                  ...
                  <div className="mb-2 flex-grow">
                    <div className="text-sm font-medium truncate">{v.conj_kanji}</div>
                    <div className="text-[10px] opacity-80 truncate font-mono">{v.conj_kana}</div>
                  </div>
                  ...
                  {v.is_active ? (
                    ...
                    <div className="text-[10px] opacity-30 mt-auto pt-1">OFFLINE</div>
                  )}
                </button>
              ))}
            </div>
